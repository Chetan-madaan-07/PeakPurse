        ...
        <div className="flex items-center gap-3 ml-auto">
          {user && (
            <div className="relative">
              ...
              {showNotifs && (
                <div className="absolute right-0 top-10 w-72 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-gray-100 dark:border-slate-800 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.map(n => (
                        <div key={n.id} className="px-4 py-3 border-b border-gray-50 dark:border-slate-800 flex items-start justify-between gap-2 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                          ...
                          <button onClick={() => markRead(n.id)} className="text-[10px] font-bold text-indigo-500 hover:text-indigo-700 uppercase">Dismiss</button>
                        </div>
                      ))
                    )}
                  </div>
